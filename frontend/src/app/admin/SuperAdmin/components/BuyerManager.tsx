...
import { Buyer, ManagerProps, API_BASE_URL } from './types';

interface BuyerManagerProps extends ManagerProps {
  onAddEmployee?: () => void;
  filterStatus?: 'approved' | 'pending' | 'all';
}

const BuyerManager: React.FC<BuyerManagerProps> = ({
  onUpdateStatus,
  onViewDetails,
  loading,
  onAddEmployee,
  ...
  const fetchBuyers = async () => {
    try {
      ...
      setError(null);
      console.log('Fetching Buyers from:', `${API_BASE_URL}/admin/buyers`);
      ...
      const errorMessage = e instanceof Error ? e.message : 'Failed to fetch buyers';
  ...
  const handleViewDetails = (buyer: Buyer) => {
    console.log('👁️ Viewing Buyer details:', {
  ...
  return (
    <div className="bg-white rounded-lg shadow">
      <div className="px-6 py-4 border-b border-gray-200">
        <h2 className="text-xl font-semibold text-gray-900">Buyer Management</h2>
        <p className="mt-1 text-sm text-gray-500">Manage buyer registrations and purchase verification</p>
      </div>
      ...
      <div className="p-6">
        ...
        {currentLoading ? (
          ...
          <div className="text-center py-8 text-gray-500">
            ...
            <div className="text-lg font-medium">No Buyers found</div>
            <div className="text-sm mt-1">Start by adding your first buyer</div>
            <button 
              onClick={fetchBuyers}
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {buyers
                  .filter(buyer => {
                    ...
                    const status = buyer.verification_status || 'pending';
                  ...
                  <tr key={buyer.id} className="hover:bg-gray-50">
                    <td className="whitespace-nowrap py-4 pl-4 pr-3 sm:pl-6">
                      <div className="flex items-center">
                        {buyer.photo_url ? (
                          <img 
                            src={`${API_BASE_URL.replace('/admin','')}${buyer.photo_url}`} 
                            alt="" 
                        ...
                        <div className="ml-4">
                          ...
                          <div className="text-gray-500 text-sm">{buyer.custom_id || ''}</div>
                        </div>
                      </div>
                    </td>
                    <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{buyer.email}</td>
                    <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                      {buyer.phone || 'N/A'}
                    </td>
                    <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                      {buyer.location || `${buyer.city || 'N/A'}, ${buyer.state || 'N/A'}`}
                    </td>
                    <td className="whitespace-nowrap px-3 py-4 text-sm">
                      <span className={`inline-flex rounded-full px-2 text-xs font-semibold leading-5 ${
                        buyer.verification_status === 'approved' || buyer.verification_status === 'verified' ? 'bg-green-100 text-green-800' : 
                        buyer.verification_status === 'rejected' ? 'bg-red-100 text-red-800' : 
                        'bg-yellow-100 text-yellow-800'
                      }`}>
                        {buyer.verification_status.charAt(0).toUpperCase() + buyer.verification_status.slice(1)}
                      </span>
                    </td>
